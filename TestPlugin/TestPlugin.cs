...
namespace TestPlugin
{
    public class TestPlugin : Plugin
    {
        ...
        public void LoadKeyboards(BaseChat chat)
        {
            ...
            keyboard.Add(new KeyboardTextButton("Menu", (c, user) => c.SendKeyboardAsync("menu")));

            //вспомогательное меню с привязкой по Id
            var childKeyboard = new Keyboard("Menu opened") { Id = "menu", InMessage = true };
            childKeyboard.Add(new KeyboardTextButton("Test1", (c, u) => c.SendMessageAsync("test1!")) { Color = ButtonColor.Red });
            childKeyboard.Add(new KeyboardTextButton("Test2", (c, u) => c.SendMessageAsync("test2!")) { Color = ButtonColor.Red });
            chat.AddKeyboard(childKeyboard);
            
            //обработчик стандартной кнопки Старт
            var startButton = new Keyboard("Start") { Id = "start" };
            childKeyboard.Add(new KeyboardTextButton("Старт", (c, u) => c.SendMessageAsync("start")));
            chat.AddKeyboard(childKeyboard);
        }
        ...
        private static void UITest1(CommandContext context)
        {
            if (context.Sender.IsAdmin)
            {
                ...
                k.AddOnNewLine(new KeyboardTextButton("test button 2", (c, u) => c.SendMessageAsync("Used by " + u.GetMentionLine())) { Color = ButtonColor.Green });
                k.Add(new KeyboardTextButton("test button 3", (c, u) => c.SendMessageAsync("Used by " + u.GetMentionLine())) { Color = ButtonColor.Blue });
